...
fn blocks_plot_frame_does_not_emit_white_background_bands() {
    ...
    let frame = render_plot_frame(&scene, PlotKind::Line, &state, Protocol::Blocks, size).unwrap();
...
fn image_protocol_plot_frame_renders_raster_payload() {
    ...
    let frame = render_plot_frame(&scene, PlotKind::Line, &state, Protocol::Kitty, size).unwrap();
